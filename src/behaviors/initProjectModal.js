...
const ORDER = ['thesis', 'visual-inspection', 'rag-chatbot', 'pneumonia', 'cognitive', 'fluenti'];
  ...
  function openModal(id){
    ...
    elMeta.innerHTML = [
      ...
      link ? metaRow(UI.source || 'SUMBER', `<a href="${link}" target="_blank" rel="noopener" class="inline-flex items-center gap-1.5 text-dim hover:text-ink transition-colors">${githubSvg}<span class="underline decoration-dim/40 underline-offset-4 hover:decoration-ink">GitHub</span></a>`) : '',
    ].join('');
